...
/*
    ...
    3.  Setter / Getter 의 장점 :
        1) 데이터 보호
            변수가 privite  접근 제어자를 사용하여 외부에서 직접 접근하지 못하게 함으로써
            데이터를 보호할 수 있음
        2) 데이터의 유효성 검증
            Setter 메서드에서 입력된 데이터가 적합한지 확인하고, 잘못된 값을 거부할 수 있음
            ex) 나이가 0보다 큰지 확인
       3) 캡슐화
            데이터를 숨기고 필요한 경우에만 제한적으로 접근할수있게 하여 클래스 내부 구형을
            외부에 노출하지 않ㅇ으

       4) 유연성 증가
        나중에 요구사항이 바뀌어 데이터를 설정하거나 조회하는 방법이 변경되더라도 클래스 내부의
       메서드만 수정하면 되므로  유지보수가 쉬워짐

   4. 그래서  Setter / Getter 가 언제나 필수적인가>
        모든 경우에  Setter / Getter 사 필요한 것은 아니지만
        일정사이즈 이상의 프로젝트를 진행해서 다수의 데이트를 입력받을 경우우
        적절한 데이터임을 검증하는 과정을 명시하기 위해서는 Setter / Getter 를 사용하는 것이 "효과적"

        거의 대부분 사용하는 개념이기 때문에 INTEELIJ에서는 ALT + INSERT 를 지원하고
        추후 수업할 LOMBOK 애너테이션을 사용하여 코드라인 자체를 줄이는 방법도 있음
        이는 객체 지향 프로그래밍에서 중요한 기초이며 클린코드를 작성하는 과정이라는 측면에서도
        중요한 개념임
